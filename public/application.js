...
var onCreateGame = function onCreateGame(event) {
  ...
  api.createGame(data).then(onGameCreated).catch(ui.createGameFailure);
};

var onGameCreated = function onGameCreated(data) {
...
var onGetGames = function onGetGames(event) {
  event.preventDefault();
  api.getGames().then(ui.showGames).catch(ui.failure);
};
...
var clickBox = function clickBox(id) {
  game.currentBoxId = id;
  var turn = game.xMove ? 'x' : 'o';
  return $.ajax({
...
var move = function move(event) {
  var id = event.target.attributes[1].value;
  var isValidMove = game.gameBoardArray[parseInt(id)] === '';
...
var checkRow = function checkRow(a, b, c, currentmove) {
  var result = false;
  var id = event.target.attributes[1].value;
  var currentMove = game.gameBoardArray[parseInt(id)];
  if (game.gameBoardArray[a] === currentMove && game.gameBoardArray[b] === currentMove && game.gameBoardArray[c] === currentMove) {
    result = true;
  }
  return result;
};

var checkWin = function checkWin(currentmove) {
  var result = false;
  var id = event.target.attributes[1].value;
  var currentMove = game.gameBoardArray[parseInt(id)];
  if (checkRow(0, 1, 2, currentMove) || checkRow(3, 4, 5, currentMove) || checkRow(6, 7, 8, currentMove) || checkRow(0, 3, 6, currentMove) || checkRow(1, 4, 7, currentMove) || checkRow(2, 5, 8, currentMove) || checkRow(0, 4, 8, currentMove) || checkRow(2, 4, 6, currentMove) || checkRow(0, 4, 8, currentMove)) {
    result = true;
    store.gameOver = true;
...
var checkDraw = function checkDraw() {
  if (game.gameBoardArray[0] !== '' && game.gameBoardArray[1] !== '' && game.gameBoardArray[2] !== '' && game.gameBoardArray[3] !== '' && game.gameBoardArray[4] !== '' && game.gameBoardArray[5] !== '' && game.gameBoardArray[6] !== '' && game.gameBoardArray[7] !== '' && game.gameBoardArray[8] !== '') {
    ui.gameDraw();
    store.gameOver = true;
  }
};
